            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {temperaments.map((t, i) => (
                  <button key={t} onClick={() => setSelectedTemp(i)} className={`px-3 py-2 rounded-lg text-xs font-medium transition-all ${i === selectedTemp ? "bg-green-500 text-white border border-green-500" : "bg-gray-50 text-gray-600 border border-gray-200 hover:border-gray-300"}`}>
                    {t}
                  </button>
                ))}
              </div>
          ...
          <div className="mb-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {allPersonas.filter(p => p.group === "franquia").map((p, i) => {
                ...
                return (
                  <div key={p.name} onClick={() => setSelectedPersona(idx)} className={`p-3 rounded-xl border transition-all duration-200 cursor-pointer ${isSelected ? "border-2 border-green-500 bg-green-50/50 shadow-sm" : "border border-gray-200 bg-white hover:border-green-300 hover:shadow-sm"}`}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {allObjections.map((obj, i) => (
              <button key={i} onClick={() => toggleObj(i)} className="w-full flex items-start gap-2 px-2.5 py-2 rounded-lg text-left hover:bg-gray-50 border border-transparent">
      ...
      {/* Right: Session Summary */}
        <div className="w-[280px] shrink-0">
          <div className="bg-white rounded-xl border border-gray-200 p-5 sticky top-8">
            ...
            <button className="w-full mt-6 flex items-center justify-center gap-2 py-3.5 bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold rounded-xl transition-colors">
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              <button className="flex items-center justify-center gap-1.5 py-2 rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors">
